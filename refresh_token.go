...
func RefreshToken(config *Config) error {
	state := config.TaskStates.SessionRefresh

	config.DefaultConfig.Log.SetState(state.Name)

	for i := 0; i < state.Retry; i++ {
		config.DefaultConfig.Log.Yellow("Refreshing session...")

		payload := strings.NewReader(fmt.Sprintf(`grant_type=refresh_token&refresh_token=%s`, config.Resources.SessionTokens.RefreshToken))
		...
		if res.StatusCode != 200 {
			...
			if res.StatusCode == 401 {
				config.DefaultConfig.Log.YellowDelay("Session can't be refreshed, proceeding to login...")
		...
		config.DefaultConfig.Log.Green("Session successfully refreshed!")
