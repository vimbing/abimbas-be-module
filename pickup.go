...
func PickupPoint(config *Config) error {
	state := config.TaskStates.Payment

	config.DefaultConfig.Log.SetState(state.Name)

	for i := 0; i < state.Retry; i++ {
		config.DefaultConfig.Log.Yellow("Filling pickup data...")

		pickupPayload := PickupPayload{
			LocationID:  "NIA01A",
			GeoLocation: GeoLocation{Longitude: 20.191064000000001, Latitude: 50.209381},
		}

		pickupPayloadJson, err := json.Marshal(pickupPayload)
		...
		req, err := http.NewRequest("PUT", fmt.Sprintf("https://api.3stripes.net/gw-api/v2/checkouts/%s/delivery_options/pick_up_point/location", config.Resources.CheckoutID), bytes.NewBuffer(pickupPayloadJson))
		...
		config.DefaultConfig.Log.Green("Pickup data filled successfully!")
