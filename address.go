...
func Address(config *Config) error {
	...
	for i := 0; i < state.Retry; i++ {
		config.DefaultConfig.Log.Yellow("Filling address data...")

		addressPayload := AddressPayload{
			LocationID: config.Resources.AddressId,
		}

		paymentPayloadJson, err := json.Marshal(addressPayload)
		...
		req, err := http.NewRequest("PUT", fmt.Sprintf("https://api.3stripes.net/gw-api/v2/checkouts/%s/delivery_options/home/location", config.Resources.CheckoutID), bytes.NewBuffer(paymentPayloadJson))
		...
		config.DefaultConfig.Log.Green("Address data filled successfully!")
