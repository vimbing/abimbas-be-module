...
func AsyncDiscount(config *Config) {
	if len(config.DefaultConfig.TaskData.Discount) < 3 {
		config.Resources.AsyncRequestsChannels.Discount <- nil
		return
	}
	...
	for i := 0; i < state.Retry; i++ {
		...
		if err != nil {
			...
			config.Resources.AsyncRequestsChannels.Discount <- definederrors.ERROR_STOP_TASK
			return
		}
		...
		if res.StatusCode != 200 {
			...
			config.Resources.AsyncRequestsChannels.Discount <- WaitAfterError{}
			return
		}
	...
	config.Resources.AsyncRequestsChannels.Discount <- config.DefaultConfig.Log.LogReturnErrorCustomText(definederrors.ERROR_TOO_MANY_RETRYS, definederrors.MESSAGE_TOO_MANY_RETRYS)
}
